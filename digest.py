...
【重要】多个信源（Product Hunt / Hacker News / Reddit）都有条目时，请尽量从多个信源各取若干，不要只选一个信源的内容。

为每个入选项填写以下字段：
- why: 这个机会解决什么问题、为什么值得关注（一句中文）
- solo_reason: 为什么判断一个人可以做到（具体说明：技术栈简单／不需要内容审核／有现成 API 等）
- signal: 条目里的客观数据（如 HN points、评论数、PH 票数；若无数字则写"无公开数据"）

按机会大小从高到低排序，最多 %d 条。如果没有真正合格的，宁可少选甚至返回空。
